...
fn main() {
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        cpu = match mem[pc] {
            ...
            0xA0 => panic!("unknown instruction 0x{:X}", mem[pc]),
            0xA1 => panic!("unknown instruction 0x{:X}", mem[pc]),
            0xA2 => panic!("unknown instruction 0x{:X}", mem[pc]),
            0xA3 => panic!("unknown instruction 0x{:X}", mem[pc]),
            0xA4 => panic!("unknown instruction 0x{:X}", mem[pc]),
            0xA5 => panic!("unknown instruction 0x{:X}", mem[pc]),
            0xA6 => panic!("unknown instruction 0x{:X}", mem[pc]),
            0xA7 => panic!("unknown instruction 0x{:X}", mem[pc]),
            0xA8 => xor_b(cpu),
            0xA9 => xor_c(cpu),
            0xAA => xor_d(cpu),
            0xAB => xor_e(cpu),
